...
const lang=localStorage.getItem('diwyashakthi-language')||document.documentElement.lang||'en';
...
async function loadEdits(){
 try{
  ...
  edits.filter(e=>e.language==='all'||e.language===lang).forEach(applyEdit);
 }catch(e){console.warn('CMS load skipped',e)}
}
function applyEdit(e){
 let nodes=[]; try{nodes=[...document.querySelectorAll(e.selector)]}catch{return}
 nodes.forEach(el=>{
  if(e.edit_type==='text') el.textContent=e.value;
  else if(e.edit_type==='attribute'&&e.attribute_name) el.setAttribute(e.attribute_name,e.value);
  else if(e.edit_type==='style') el.setAttribute('style',e.value);
  else el.innerHTML=e.value;
 });
}
...
function enablePreview(){
 ...
 const st=document.createElement('style'); st.textContent='.cms-preview-mode body *{cursor:crosshair!important}.cms-preview-mode [data-cms-hover]{outline:2px solid #e2ab3c!important;outline-offset:2px}.admin-entry{display:none!important}'; document.head.appendChild(st);
 document.addEventListener('mouseover',e=>{const t=e.target.closest('main *,header *,footer *'); if(t)t.setAttribute('data-cms-hover','1')});
 document.addEventListener('mouseout',e=>{const t=e.target.closest('[data-cms-hover]');if(t)t.removeAttribute('data-cms-hover')});
 document.addEventListener('click',e=>{const t=e.target.closest('main *,header *,footer *'); if(!t||t.closest('a,button')&&e.metaKey)return; e.preventDefault();e.stopPropagation();
  parent.postMessage({type:'cms-select',page,selector:selectorFor(t),tag:t.tagName.toLowerCase(),html:t.innerHTML,text:t.textContent.trim().slice(0,500),src:t.getAttribute('src')||'',href:t.getAttribute('href')||'',style:t.getAttribute('style')||''},location.origin);
 },true);
}

addAdminEntry();
window.addEventListener('DOMContentLoaded',()=>setTimeout(loadEdits,50));
window.addEventListener('diwy-language-changed',()=>loadEdits());
